...
from src.domain.retrieval.item_verdict import Accepted, citations_of
...
def test_make_evaluated_answer_is_grounded_by_default() -> None:
    ...
    assert citations_of(answer.verdict)
    assert {
        c.url for c in citations_of(answer.verdict)
    } <= answer.retrieved_source_urls
...
def test_make_answer_audit_record_is_grounded_by_default() -> None:
    ...
    assert {
        c.url for c in citations_of(record.verdict)
    } <= record.retrieved_source_urls
