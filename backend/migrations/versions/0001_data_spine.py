...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.text("now()")),
        sa.CheckConstraint("type IN ('city', 'county', 'state')",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["material_id"], ["materials.id"],
                                ...
                                ondelete="RESTRICT"),
        sa.UniqueConstraint("material_id", "alias",
    ...
    op.create_table(
        ...
        sa.Column("preparation_steps", postgresql.ARRAY(sa.String()), nullable=False,
                  server_default=sa.text("ARRAY[]::text[]")),
        sa.Column("exceptions", postgresql.ARRAY(sa.String()), nullable=False,
                  server_default=sa.text("ARRAY[]::text[]")),
        ...
        sa.CheckConstraint(
            "disposition IN ('curbside_recycle', 'dropoff', 'compost', 'landfill', "
        ...
        sa.ForeignKeyConstraint(["source_document_id"], ["source_documents.id"],
                                ...
                                ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["superseded_by"], ["rules.id"],
    ...
    op.create_table(
        ...
        sa.Column("must_cite_source", sa.Boolean(), nullable=False,
                  server_default=sa.text("true")),
        sa.Column("refusal_required", sa.Boolean(), nullable=False,
                  server_default=sa.text("false")),
        sa.Column("notes", sa.Text(), nullable=True),
        sa.ForeignKeyConstraint(["jurisdiction_id"], ["jurisdictions.id"],
                                name="fk_regression_cases_jurisdiction_id",
                                ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["expected_material_id"], ["materials.id"],
    ...
    op.create_table(
        ...
        sa.Column("normalized_materials",
                  postgresql.ARRAY(postgresql.UUID(as_uuid=True)), nullable=False,
                  server_default=sa.text("ARRAY[]::uuid[]")),
        sa.Column("retrieved_rule_ids",
                  postgresql.ARRAY(postgresql.UUID(as_uuid=True)), nullable=False,
                  server_default=sa.text("ARRAY[]::uuid[]")),
        sa.Column("retrieved_source_ids",
                  postgresql.ARRAY(postgresql.UUID(as_uuid=True)), nullable=False,
                  server_default=sa.text("ARRAY[]::uuid[]")),
        sa.Column("prompt_name", sa.String(), nullable=False),
        sa.Column("prompt_version", sa.Integer(), nullable=False),
        sa.Column("model_id", sa.String(), nullable=False),
        sa.Column("raw_model_output", postgresql.JSONB(), nullable=False,
                  server_default=sa.text("'{}'")),
        sa.Column("final_answer", postgresql.JSONB(), nullable=False,
                  server_default=sa.text("'{}'")),
        sa.Column("validator_result", postgresql.JSONB(), nullable=False,
                  server_default=sa.text("'{}'")),
