...
logger.warning("Cache miss for resource %s, fetching from source", resource_id)
...
async def get_resource(resource_id: int):
    ...
    logger.info("Resource %s: Response size %s bytes", resource_id, len(response_json))
    return response
```
...
logger.info("External service response: status=%s, duration=%.2fs", response.status_code, duration)
```
...
logger.error("Database error fetching resource %s: %s", resource_id, str(e)[:200])
```
