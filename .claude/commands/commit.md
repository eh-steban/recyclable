...
1. Run `git status` (never -uall) and `git diff` (staged + unstaged) to
   understand what changed
2. Run `git log --oneline -5` to match recent commit message style
3. Draft a commit message:
   - Subject line: `<type>[optional scope]: <description>` -- MUST be ≤72
     chars
   ...
   - Breaking changes: append `!` after type, or use `BREAKING CHANGE:`
     footer
